...
export default function App() {
  ...
  return (
    <main className="app">
      ...
      <footer className="actions"><button className="restart-btn" onClick={() => { gRef.current?.restart(); setPaused(false); }}>Restart</button><button className="restart-btn" onClick={() => { const ok = paused ? gRef.current?.resume() : gRef.current?.pause(); if (ok) setPaused((v) => !v); }}>{paused ? "Resume" : "Pause"}</button><button className="restart-btn" disabled={hud.special > 0 || paused} onClick={() => gRef.current?.triggerSpecial()}>Special Pulse</button><p>Boss waves every 4 levels. Use Special Pulse wisely.</p></footer>
