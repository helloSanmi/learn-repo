...
export default function App() {
  ...
  const [hud, setHud] = useState({ score: 0, level: 1, lives: 3, time: 45, powerUp: "None", combo: 0 });
  ...
  return (
    <main className="app">
      ...
      <section className="stats"><article><p>Score</p><strong>{hud.score}</strong></article><article><p>Best</p><strong>{best}</strong></article><article><p>Combo</p><strong>x{1 + Math.floor(hud.combo / 3)}</strong></article><article><p>Level</p><strong>{hud.level}</strong></article><article><p>Lives</p><strong>{hud.lives}</strong></article><article><p>Time</p><strong>{hud.time}s</strong></article><article><p>Power-Up</p><strong>{hud.powerUp}</strong></article></section>
      ...
      <footer className="actions"><button className="restart-btn" onClick={() => { gRef.current?.restart(); setPaused(false); }}>Restart</button><button className="restart-btn" onClick={() => { const ok = paused ? gRef.current?.resume() : gRef.current?.pause(); if (ok) setPaused((v) => !v); }}>{paused ? "Resume" : "Pause"}</button><p>Pause, resume, or restart anytime.</p></footer>
